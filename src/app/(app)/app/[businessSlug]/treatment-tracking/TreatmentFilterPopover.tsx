...
type Filters = { from: string; to: string; sku: string; target: string; product: string };
const empty: Filters = { from: "", to: "", sku: "", target: "", product: "" };
...
const KEYS: (keyof Filters)[] = ["from", "to", "sku", "target", "product"];
...
export function TreatmentFilterPanel({
  isOpen,
  filters,
  setFilters,
  onApply,
    ...
    <div className="absolute right-0 top-12 z-20 w-80 rounded-md border border-gray-200 bg-white p-4 shadow-lg">
      <div className="space-y-3 text-sm text-gray-700">
        ...
        <label className="flex flex-col gap-1">
          ...
          <input type="text" className="rounded-md border border-gray-300 px-2 py-1.5" value={filters.sku} onChange={(e) => setFilters((p) => ({ ...p, sku: e.target.value }))} />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Target</span>
          <input type="text" className="rounded-md border border-gray-300 px-2 py-1.5" value={filters.target} onChange={(e) => setFilters((p) => ({ ...p, target: e.target.value }))} />
        </label>
        <label className="flex flex-col gap-1">
