...
type Filters = { from: string; to: string; vendor: string; category: string; sku: string };
const empty: Filters = { from: "", to: "", vendor: "", category: "", sku: "" };
...
const KEYS: (keyof Filters)[] = ["from", "to", "vendor", "category", "sku"];

export function useProductIntakeFilter() {
...
export function ProductIntakeFilterPanel({
  isOpen,
  filters,
  setFilters,
  onApply,
    ...
    <div className="absolute right-0 top-12 z-20 w-80 rounded-md border border-gray-200 bg-white p-4 shadow-lg">
      <div className="space-y-3 text-sm text-gray-700">
        ...
        <label className="flex flex-col gap-1">
          ...
          <input type="text" className="rounded-md border border-gray-300 px-2 py-1.5" placeholder="Filter by vendor" value={filters.vendor} onChange={(e) => setFilters((p) => ({ ...p, vendor: e.target.value }))} />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Category</span>
          <input type="text" className="rounded-md border border-gray-300 px-2 py-1.5" placeholder="Filter by category" value={filters.category} onChange={(e) => setFilters((p) => ({ ...p, category: e.target.value }))} />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">SKU</span>
          <input type="text" className="rounded-md border border-gray-300 px-2 py-1.5" placeholder="Filter by SKU" value={filters.sku} onChange={(e) => setFilters((p) => ({ ...p, sku: e.target.value }))} />
        </label>
