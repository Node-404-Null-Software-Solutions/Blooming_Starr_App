...
export default function SkuScannerClient({
  ...
  function attachStreamWatchers(stream: MediaStream) {
    clearStreamWatchers();

    const cleanupCallbacks = stream.getVideoTracks().flatMap((track) => {
      const trackLabel = track.label || "Camera";

      const handleEnded = () => {
        if (isStoppingRef.current) return;

        refreshDiagnostics("ended", `${trackLabel} ended unexpectedly.`);
        setError(
          "The camera stopped unexpectedly. Close other apps or browser tabs using the camera, then start it again."
        );
        stopScanner();
      };
      const handleMute = () => {
        ...
        refreshDiagnostics("muted", `${trackLabel} stopped sending video frames.`);
        setStatus("Camera feed paused by the browser. Keeping the scanner ready...");
      };
      const handleUnmute = () => {
        if (isStoppingRef.current) return;

        refreshDiagnostics("granted");
  ...
  function refreshDiagnostics(liveCameraStatus?: string, lastError?: string) {
    ...
    const nextDiagnostics = {
      ...
      liveCameraStatus: liveCameraStatus ?? "not checked",
      lastError,
    };
    ...
    setDiagnostics((previous) => ({
      ...nextDiagnostics,
      liveCameraStatus:
        liveCameraStatus ?? previous?.liveCameraStatus ?? nextDiagnostics.liveCameraStatus,
  ...
  async function startScanner() {
    ...
    setRawValue("");
    setStatus("");
    refreshDiagnostics("checking");

    if (!window.isSecureContext || !navigator.mediaDevices?.getUserMedia) {
      refreshDiagnostics("blocked", "Live camera requires HTTPS or localhost.");
      setError(
    ...
    try {
      ...
      const scanner = new QrScanner(
        ...
        },
        {
          maxScansPerSecond: 8,
          onDecodeError: () => {
            // No QR in the current frame is expected while the user is aiming.
          },
      ...
      refreshDiagnostics(
        "blocked",
        cameraError instanceof Error ? `${cameraError.name}: ${cameraError.message}` : String(cameraError)
      );
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            ...
            {diagnostics && (
              <div className="mt-3 rounded-md bg-gray-50 px-3 py-2 text-xs text-gray-600">
                ...
                <p>Live camera check: {diagnostics.liveCameraStatus}</p>
                {diagnostics.lastError && <p>Last camera error: {diagnostics.lastError}</p>}
              </div>
            )}
          </div>
